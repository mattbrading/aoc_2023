...
fn calculate_range(duration: &u64, min_distance: &u64) -> Range<u64> {
...
fn parse_input(input: &str) -> Result {
    ...
    let dists = dist_str.split_whitespace().skip(1)
        .map(|str| str.parse::<u64>().unwrap());

    let races = times.zip(dists);

    let part_1 = sum_winning_options(races.collect());

    let combined_time = time_str.split_whitespace().skip(1)
        .collect::<Vec<&str>>()
        .join("")
        .parse::<u64>()
        .unwrap();

    let combined_dist = dist_str.split_whitespace().skip(1)
        .collect::<Vec<&str>>()
        .join("")
        .parse::<u64>()
        .unwrap();
...
#[cfg(test)]
mod tests {
    use crate::{calculate_range, count_winning_options, sum_winning_options, parse_input};
    ...
    fn test_calculate_range () {
        ...
        for (duration, min_distance, expected) in races {
            let result = calculate_range(&duration, &min_distance);
    ...
    fn test_sum_winning_options () {
        ...
        let result = sum_winning_options(races);
